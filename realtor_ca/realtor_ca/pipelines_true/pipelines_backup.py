...
class StreetAddressWriterPipeline:
    ...
    def open_spider(self, spider):
        if spider.name in ["duproprio_fr", "kijiji_fr", "lespac_fr", "publimaison_fr", "logisqc_fr", "annoncextra_fr"]:
            self.file = open(f'street_addresses_{datetime.now().strftime("%Y-%m-%d")}.txt', 'w', encoding='utf-8')
...
class MongoDBPipeline:
    ...
    def from_crawler(cls, crawler):
        ...
        pipeline = cls(mongo_uri=mongo_uri, mongo_db=mongo_db, collection_name=collection_name)
        pipeline.db = pipeline.client[pipeline.mongo_db]  # Initialization of the db attribute
        return pipeline
    ...
    def close_spider(self, spider):
        # if "centris" not in spider.name:
        #     avpp_items = self.items.get(spider.name, [])
    ...
    def close_mongodb(self):
        try:
            self.client.admin.command('ping')  # Vérifier que la connexion est toujours active
            self.save_data()  # Si nous sommes ici, la connexion est toujours active, donc on peut sauvegarder les données.
        except pymongo.errors.PyMongoError:
            # La connexion a échoué, ou une autre erreur liée à MongoDB s'est produite.
            pass
        finally:
            self.client.close()  # Fermer la connexion.

    def signal_handler(self, signal, frame):
        # if "centris" not in self.spider_name:
    ...
    def load_data(self):
        if self.db is not None:
            ...
            # self.load_centris_data()  # Ajout de cette ligne pour charger les données Centris

    # def load_centris_data(self):
    #     if self.db is not None:
    #         if self.collection_name is not None and "centris" in self.collection_name:
    #             centris_collection = self.db[self.collection_name]
    #             self.centris_data = list(
    #                 centris_collection.find({}, {'latitude': 1, 'longitude': 1, 'street_address': 1})
    #             )
    #             print("Données Centris chargées :")
    #             print(self.centris_data)
    #         else:
    #             print("Pas de données Centris à charger.")
        ...
        if self.file_name_avpp is not None and "avpp" in self.file_name_avpp:
            ...
            if centris_items:
                ...
                with centris_lock:
                    ...
                    self.db[self.backup_collection_name].insert_many(centris_items)
    ...
    #     result = {
    #         'centris - latitude': False,
    #         'centris - longitude': False,
    #         'centris - street_address': False
    #     }
    ...
    #             truncated_latitude = round(float(latitude), 8)
    #             truncated_longitude = round(float(longitude), 8)

    #             for centris_item in self.centris_data:
    #                 centris_latitude = round(float(centris_item['latitude']), 8)
    #                 centris_longitude = round(float(centris_item['longitude']), 8)
    #                 lat_diff = abs(centris_latitude - truncated_latitude)
    #                 lon_diff = abs(centris_longitude - truncated_longitude)

    #                 if lat_diff < 1e-8 and lon_diff < 1e-8:
    #                     result['centris - latitude'] = True
    #                     result['centris - longitude'] = True
    #                     print(f"Coordonnées Centris en double trouvées : Latitude={latitude}, Longitude={longitude}")
    #                     break

    #         if street_address is not None:
    #             result['centris - street_address'] = street_address in [item['street_address'] for item in self.centris_data]
    ...
    def process_item(self, item, spider):
        ...
        # if "centris" not in spider.name:
        #     item_data.update(self.is_centris_duplicate(item_data))
