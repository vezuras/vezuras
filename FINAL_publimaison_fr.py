...
class PUBLIMAISON_FRSpider(scrapy.Spider):
    name = "publimaison_fr"
    ...
    def __init__(self, *args, **kwargs):
        super(PUBLIMAISON_FRSpider, self).__init__(*args, **kwargs)
    ...
    def parse_summary(self, response):
        ...
        if titre:  # Vérifier si le titre existe
            ...
            annonce = {
                ...
                'latitude': '',
                'longitude': '',
                'phone': [],  # Utiliser une liste pour stocker les numéros de téléphone
            ...
            for element in telephone_elements:
                hash_values = element.xpath("./@data-url").getall()
    ...
    def parse_telephones(self, response):
        try:
            ...
            if phone_number:
                ...
                annonce = response.meta['annonce']
                annonce['phone'].extend(formatted_phone)  # Utiliser la méthode extend() pour ajouter les numéros à la liste

                self.annonces.append(annonce)
        except json.JSONDecodeError:
            self.logger.error("Erreur de décodage JSON: %s", response.text)
